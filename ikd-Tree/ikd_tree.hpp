...
class KD_TREE {
 public:
  ...
  struct KD_TREE_NODE {
    ...
    double node_range_x[2], node_range_y[2], node_range_z[2];
 ...
 private:
  ...
  int Treesize_tmp = 0;
  double alpha_bal_tmp = 0.5, alpha_del_tmp = 0.0;
  double delete_criterion_param = 0.5f;
  double balance_criterion_param = 0.7f;
  double downsample_size = 0.2f;
  ...
  void Search(KD_TREE_NODE* root, int k_nearest, PointType point, MANUAL_HEAP& q, double max_dist) {
    ...
    if (!root->point_deleted) {
      double dist = calc_dist(point, root->point);
    ...
    double dist_left_node = calc_box_dist(root->left_son_ptr, point);
    double dist_right_node = calc_box_dist(root->right_son_ptr, point);
  ...
  void Search_by_radius(KD_TREE_NODE* root, PointType point, double radius, PointVector& Storage) {
    if (root == nullptr) return;
    Push_Down(root);
  ...
  bool Criterion_Check(KD_TREE_NODE* root) {
    ...
    double delete_evaluation = 0.0f;
    KD_TREE_NODE* son_ptr = root->left_son_ptr;
    if (son_ptr == nullptr) son_ptr = root->right_son_ptr;
    delete_evaluation = double(root->invalid_point_num) / root->TreeSize;
    balance_evaluation = double(son_ptr->TreeSize) / (root->TreeSize - 1);
  ...
  void Update(KD_TREE_NODE* root) {
    ...
    double tmp_range_z[2] = {INFINITY, -INFINITY};
    // Update Tree Size
    ...
    double z_L = (root->node_range_z[1] - root->node_range_z[0]) * 0.5;
    ...
    if (root == Root_Node && root->TreeSize > 3) {
      KD_TREE_NODE* son_ptr = root->left_son_ptr;
      if (son_ptr == nullptr) son_ptr = root->right_son_ptr;
      double tmp_bal = double(son_ptr->TreeSize) / (root->TreeSize - 1);
 ...
 public:
  ...
  void Set_delete_criterion_param(double delete_param) { delete_criterion_param = delete_param; };
  void Set_balance_criterion_param(double balance_param) { balance_criterion_param = balance_param; };
  void set_downsample_param(double box_length) { downsample_size = box_length; };
  ...
  void root_alpha(double& alpha_bal, double& alpha_del) {
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
  ...
  void Nearest_Search(PointType point, int k_nearest, PointVector& Nearest_Points, vector<double>& Point_Distance,
  ...
  void Radius_Search(PointType point, const double radius, PointVector& Storage) {
    Storage.clear();
